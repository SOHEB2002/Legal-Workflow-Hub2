...
- **Phase 8 Batch P8-C — DONE (9876f4b, committed, NOT yet pushed):** cosmetic F14 (awaitCaseCompletion uses `CaseStage.DATA_COMPLETION` enum, byte-identical refactor) + F15 (corrected the consultations.source default comment to `عبر_المجموعة`).
- **Phase 8 deferred decisions:**
  - **F7 (legacy memo review panel, memos.tsx ~1916):** delete-if-dead pending a PRODUCTION row count of `memos WHERE current_stage IS NULL` (0 → delete; >0 → keep). Count must be run against prod (neondb) by the user — this machine has only dev DB access and DATABASE_URL is permission-denied.
