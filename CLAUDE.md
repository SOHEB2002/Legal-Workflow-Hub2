...
- **Phase 3 (storage.ts):** IN PROGRESS, branch only (NOT deployed). 122→107 casts so far.
  ...
  - **3E** PROPOSED, not applied — decision was "memo follows its parent case's lawyer". Investigation: at dept-transfer the case's own lawyers are cleared in the same PATCH (routes ~2148-2150), so there is no case lawyer to point at *at transfer time*; the follow-the-case semantic is already implemented by the existing primaryLawyerId-change cascade (routes ~2449-2468) which re-points active memos when the new dept assigns a lawyer. Proposal: at transfer set `assignedTo: ""` (the de-facto unassigned sentinel used by auto-memos `primaryLawyerId || responsibleLawyerId || ""`; UI + permission checks already guard `!!assignedTo`), delete the broken null-write, rely on the cascade for the reassignment leg. Awaiting user approval.
- **Deferred / pending:**
