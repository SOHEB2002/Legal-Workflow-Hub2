...
- Sentinels: memo unassignment writes `""` (never null).
- **Canonical lawyer resolution — SETTLED by the owner 2026-08-04, one order everywhere: `primaryLawyerId || responsibleLawyerId || ""`.** Use the shared `caseNotificationRecipientId(lawCase)` helper (shared/schema.ts) rather than re-writing the chain. `responsibleLawyerId` has NO UI input — all three assignment controls are labelled "المحامي المسؤول" and write `primaryLawyerId`, two of them exclusively — so after a reassignment `responsibleLawyerId` names the SUPERSEDED lawyer. It stops being written in the field-convergence batch 3; the column stays (additive-only). **The only sanctioned exceptions are the hearing-attendance chains** (`litigatorId || primary || responsible` for a new hearing's `attendingLawyerId`, and `hearing.attendingLawyerId || <case lawyer>` for hearing reminders and the صك task) — those answer "who APPEARS in court", not "who do we notify".
