...
// ⚠ ensureObjectionMemoForCase is imported, NOT reimplemented — see
// checkObjectionMemoDue. It is the same creator POST /api/cases/:id/judgment-deed
// calls, so a memo raised by the nightly sweep is byte-identical to one raised at
// the counter.
import { calculateSmartPriority, recomputePrescriptionForCase, ensureObjectionMemoForCase } from "./routes";
...
export function startScheduler() {
  ...
  cron.schedule("0 8 * * *", async () => {
    ...
    // Batch 8 — raises the لائحة اعتراضية for any ruling whose صك date has
    // ARRIVED. Deliberately LAST: it only reads judgment rows and writes memos,
    // so nothing above it depends on its output, and a failure inside it cannot
    // shorten the rest of the morning's run.
    await checkObjectionMemoDue();
  });
...
// (routes.ts, the تم_تزويد_العميل_بصيغة_التظلم default).
...
// ==================== لائحة اعتراضية — THE DEFERRED CREATION (batch 8) ====================
// 🔴 THIS JOB EXISTS BECAUSE THE MEMO MOVED OFF THE TYPING DATE AND ONTO THE
// ARRIVAL DATE. Recording a صك receipt date that is still in the FUTURE used to
// raise the لائحة اعتراضية immediately, so it sat in the lawyer's list as work
// that could not start — the document had not been received. A future date is a
// legitimate EXPECTED date (owner ruling: it is NOT a data error, and nothing
// anywhere rejects one), so the fix is not a guard on the input; it is to create
// the memo ON the date. POST /api/cases/:id/judgment-deed now returns "deferred"
// for a future date and creates nothing, and this run picks the ruling up on the
// morning its date arrives.
//
// 🔴 ONE CREATOR, REUSED — NOT A SECOND IMPLEMENTATION. This calls
// ensureObjectionMemoForCase (server/routes.ts), the same function the deed route
// calls, so the memo's type, title, description, priority, assignment, createdBy,
// isAutoGenerated/autoGenerateReason, the per-ruling dedup and the activeMemoCount
// refresh are all IDENTICAL to a memo created at the counter. The function needed
// no restructuring to be callable here: it takes plain data and touches nothing
// request-scoped, and scheduler.ts already imports from ./routes
// (calculateSmartPriority, recomputePrescriptionForCase).
//
// 🔴 THE DEADLINE IS READ, NEVER RECOMPUTED. It comes from the judgment row's
// stored objection_deadline — the value recordDeedReceipt computed from
// receiptDate + window when the receipt was recorded. Deriving it from the
// creation date instead would silently move a legal deadline, and this job runs
// days after the arithmetic was done.
//
// TWO IDEMPOTENCE GUARDS, answering two different questions:
//   1. "does the memo already exist?" — ensureObjectionMemoForCase's own
//      per-ruling lookup. A re-run finds it and returns "skipped"/"updated";
//      it can never mint a second one.
//   2. "was it ever raised?" — the case_activity_log marker
//      (objection_memo_raised + related_entity_id = the judgment). It OUTLIVES the
//      memo, because DELETE /api/memos/:id is a hard delete, so a memo a branch
//      manager deleted on purpose is never re-created. This is checked HERE rather
//      than inside the creator so the deed route keeps its exact current behaviour
//      at the counter — a user re-recording a receipt is a deliberate act and must
//      still be able to re-raise a memo they removed.
//
// ⚠ WHAT THE MARKER CANNOT DO — and it is why ObjectionMemoSweepEpoch exists.
// Rulings whose memo was deleted BEFORE this batch shipped have no marker, so for
// them "deleted on purpose" and "never created" really are indistinguishable. The
// epoch resolves that WITHOUT guessing: before this batch, the deed route created
// the memo unconditionally at receipt-recording time, for every date, so EVERY
// objectionable ruling with a deed date already had its memo raised. For a ruling
// dated before the epoch, absence therefore MEANS deletion — and the job skips it
// permanently. This keeps the sweep forward-only instead of a one-time historical
// resurrection on first deploy.
//
// 🔴 SET THE EPOCH TO THE DAY THIS SHIPS, AND NEVER MOVE IT FORWARD LATER. Too
// early only risks re-raising a memo deleted during the deploy lag; too LATE
// silently skips real arrivals, which is the failure that has no surface.
const ObjectionMemoSweepEpoch = "2026-08-22";

async function checkObjectionMemoDue() {
  try {
    // Resolved ONCE so every candidate in the sweep is judged against the same
    // calendar boundary — Asia/Riyadh via Intl, never a toISOString slice.
    const today = firmToday();
    const candidates = await storage.getJudgmentsAwaitingObjectionMemo(today, ObjectionMemoSweepEpoch);
    if (candidates.length === 0) return;

    const alreadyRaised = await storage.getJudgmentIdsWithObjectionMemoActivity(
      candidates.map((j) => j.id),
    );

    let created = 0;
    for (const judgment of candidates) {
      if (alreadyRaised.has(judgment.id)) continue;
      // Per-ruling try/catch: one broken case must not abort the rest of the
      // sweep, and the next run retries it.
      try {
        const lawCase = await storage.getCaseById(judgment.caseId);
        if (!lawCase) continue;
        // 🔴 A CLOSED CASE GETS NO NEW MEMO. Closing a case CANCELS its open memos
        // (cancelOpenCaseChildrenOnClose), so raising a fresh active one days later
        // would resurrect work on a dead case and bump its activeMemoCount. The
        // deed route cannot hit this — it runs at the counter while the case is at
        // a judgment stage — but this job runs days after the receipt was typed,
        // by which time the case may have moved. Conservative in the only safe
        // direction: it can create FEWER memos, never more. A case that is
        // reopened before the window closes is picked up again on the next run,
        // because nothing was marked.
        if (lawCase.currentStage === "مقفلة") continue;

        const deadline = String(judgment.objectionDeadline || "").trim();
        const deedDate = String(judgment.deedReceivedDate || "").trim();
        // Both are guaranteed non-null by the query's own conditions; the guard is
        // belt-and-braces against a future widening of that query.
        if (!deadline || !deedDate) continue;

        const outcome = await ensureObjectionMemoForCase(lawCase, {
          deadline,
          judgmentType: String(judgment.outcome || ""),
          hearingId: judgment.hearingId,
          judgmentId: judgment.id,
          deedReceivedDate: deedDate,
          // The established actor sentinel. case_activity_log.user_id carries no
          // FK, and "system" is what the auto-flag sweep and every auto-generated
          // memo already write.
          actor: { id: "system", name: "النظام" },
        });
        if (outcome.action === "created") created++;
      } catch (e) {
        console.error("[checkObjectionMemoDue] judgment", judgment.id, "failed:", e);
      }
    }
    if (created > 0) {
      console.log(`[checkObjectionMemoDue] raised ${created} لائحة اعتراضية memo(s) whose صك date arrived`);
    }
  } catch (error) {
    console.error("Error checking due objection memos:", error);
  }
}
