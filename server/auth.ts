...
export function validatePassword(password: string): { valid: boolean; message: string } {
  ...
  if (!/[A-Za-z\u0621-\u064A]/.test(password)) return { valid: false, message: "يجب أن تحتوي على حروف" };
  if (!/[A-Z]/.test(password)) return { valid: false, message: "يجب أن تحتوي على حرف كبير إنجليزي واحد على الأقل" };
  if (!/[0-9]/.test(password)) return { valid: false, message: "يجب أن تحتوي على أرقام" };
  if (!/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password)) return { valid: false, message: "يجب أن تحتوي على رمز خاص واحد على الأقل (!@#$%...)" };
