            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {/* 🔴 BATCH 17 — A TYPE THAT WOULD STRAND THIS MEMO IS
                        DISABLED, not offered and then refused. memo_type selects
                        the PATH, so retyping a memo sitting on مراجعة_داخلية /
                        لجنة_مراجعة / الأخذ_بالملاحظات to «أخرى» would leave its
                        current stage off its own new path, with no edge out.
                        Same containment test as the server's 400 — "is this
                        memo's CURRENT stage on the path this type resolves to" —
                        so the two cannot disagree, and it is general rather than
                        an «أخرى» special case. Retyping AWAY from «أخرى» is
                        always allowed: the short path's stages are all on the
                        long one. */}
                    {Object.entries(MemoTypeLabels).map(([value, label]) => {
                      const stranded = !!editMemoDialog.currentStage
                        && value !== editMemoDialog.memoType
                        && !getMemoStagePath(value, { includeTakingNotes: true })
                          .includes(editMemoDialog.currentStage as MemoStageValue);
                      return (
                        <SelectItem key={value} value={value} disabled={stranded}>
                          {stranded ? `${label} — غير متاح في هذه المرحلة` : label}
                        </SelectItem>
                      );
                    })}
                  </SelectContent>
